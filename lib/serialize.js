const { getContentType, extractMessageContent, downloadMediaMessage } = require("baileys");
...
const serialize = async (msg, client) => {
    ...
    if (msg.message?.[m.type]?.contextInfo?.quotedMessage) {
        ...
        m.quoted.download = (type = "buffer") => downloadMedia(m.quoted, type, client);
    } else {
        m.quoted = null;
    }
    if (m.type) {
    ...
    // m.groupMetadata = async () => (m.isGroup ? await client.groupMetadata(m.from) : null);
    ...
    m.download = (type = "buffer") => downloadMedia(msg, type, client);
    return m;
};

async function downloadMedia(m, type, client) {
